...
export default function RepositoryPage() {
  ...
  const [reportTheme, setReportTheme] = useState('all');
  ...
  const filteredReports = useMemo(() => {
    return reports.filter(r => {
      ...
      const matchesTheme = reportTheme === 'all' || r.theme === reportTheme;
      return matchesSearch && matchesTheme;
    });
  }, [reports, reportSearch, reportTheme]);
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {reportTheme !== 'all' && (
                      <motion.span
                        ...
                        className="inline-flex items-center px-3 py-1 rounded-full bg-primary-100 text-primary-700 text-sm"
                      >
                        Theme: {themes.find(t => t.key === reportTheme)?.label}
                        <button
                          onClick={() => setReportTheme('all')}
                          className="ml-2 hover:text-primary-900"
                        >
                          <X className="w-4 h-4" />
                        </button>
