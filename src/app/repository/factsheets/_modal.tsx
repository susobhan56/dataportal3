...
import factsheetsData from '@/data/factsheets.json';
...
const factsheets: Factsheet[] = factsheetsData as Factsheet[];
import FactsheetCard from '@/components/FactsheetCard';
import FileViewer from '@/components/FileViewer';
import { useState } from 'react';

export default function RepositoryFactsheetsPage() {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {factsheets.map((item) => (
            <FactsheetCard
              ...
              onView={() => handleView(item.file)}
              onDownload={() => window.open(item.file, '_blank')}
            />
          ))}
        </div>
