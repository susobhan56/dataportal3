...
export default function HealthThemePage() {
  const theme = themes.find((t) => t.slug === 'health');
  const themeContent = recentContent.filter((content) => content.theme === 'health');
      ...
      <section className="container mx-auto px-4 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {themeContent.map((item) => (
            <Link key={item.id} href={`/themes/health/${item.id}`}>
